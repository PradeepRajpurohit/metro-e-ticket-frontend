...
//     }
    ...
    return (
        <div className='h-24 flex items-center justify-between px-20 shadow-md'>

            <img src={metro} alt=""></img>

            <ul className='flex space-x-14 text-red-600 font-semibold'>
            ...
            {localStorage.getItem("authToken")?<div className="relative inline-block">
                <button onClick={handleUserShow} className="bg-red-600 text-white w-16 h-16 text-4xl flex items-center justify-center rounded-full cursor-pointer">P</button>
                <div className="transition-all absolute bg-white min-w-fit z-10 p-4 shadow-lg right-0 mt-6" style={{ "display": `${show}` }}>
                    <div className='flex space-x-2 items-center'>
                        <div className="bg-red-600 text-white w-16 h-16 text-4xl flex items-center justify-center rounded-full cursor-pointer">P</div>
                        <div>
                            <p>{user.name}</p>
                            <p>{user.email}</p>
                        </div>
            ...
            <div className='flex space-x-6 text-white'>
                ...
                <Link className='transition-all inline-block font-semibold bg-red-600 px-6 py-1 rounded hover:text-red-600 border-red-600 border-[1px] hover:bg-white' to='/signup'>Sign Up</Link>
            </div>}
